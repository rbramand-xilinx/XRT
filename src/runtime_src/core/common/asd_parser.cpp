...
get_formated_tiles_info(const xrt_core::device* device, aie_tile_type& tile_type, aie_tiles_info& info)
{
  ...
  if (!((info.major == aie_parser::aie_tiles_info_version_major) && (info.minor == aie_parser::aie_tiles_info_version_minor)))
  ...
  switch (tile_type) {
    case aie_tile_type::core :
      auto aie_cols = parse_data_from_buf<aie_core_tile_status>(buf.data(), info);
      ...
      pt.put("schema_version.major", major_ver);
      pt.put("schema_version.minor", minor_ver);
