...
struct aie_cols_status_info
{
  ...
  static result_type
  get(const xrt_core::device* dev, key_type key, const boost::any& param)
  {
    auto data = boost::any_cast<xrt_core::query::aie_cols_status_info::meta_data>(arg);

    std::vector<char> buf(data.info.col_size * data.num_cols, 0);

    xocl::shim *drv = xocl::shim::handleCheck(dev->get_user_handle());
    if(!drv)
      throw std::runtime_error("Unable to get shim handle of the device");
      
    drv->get_aie_col_info(buf.data(), data.info.col_size * data.num_cols, data.start_col, data.num_cols);
